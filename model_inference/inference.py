...
def get_embedder(model_name: str):
    ...
    if model_name not in MODEL_MAPPING:
        raise ValueError(f"Model '{model_name}' is not configured in config.py. Available models: {list(MODEL_MAPPING.keys())}")
...
def main():
    parser = argparse.ArgumentParser(description="Generate image embeddings for edge AI tasks.")
    parser.add_argument("--image", type=str, required=True, help="Path to input image file.")
    parser.add_argument("--model", type=str, default=DEFAULT_MODEL, choices=list(MODEL_MAPPING.keys()),
                        help=f"Model key to use for embedding extraction (default: {DEFAULT_MODEL}).")
    ...
    try:
        ...
        try:
            ...
            print(f"Error: Unable to open image file '{args.image}'. Details: {e}", file=sys.stderr)
            sys.exit(1)
